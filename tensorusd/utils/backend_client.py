...
class BackendClient:
    ...
    def upload_agent_output(
        ...
        """
        POST /submission/agent-output

        Upload the CSV output generated by an agent evaluation.

        *csv_bytes*       — the CSV content as raw bytes.
        *agent_filename*  — the original submitted Python filename (must end with .py).
        *csv_filename*    — optional explicit CSV filename (default: derived from *agent_filename*).

        Returns the response data dict with keys: file_id, file_path, filename, eval_date.
        """
        ...
        try:
            r = self._session.post(
                f"{self._base}/submission/agent-output",
                files=files,
                data=data,
                timeout=settings.http_timeout,
            )
